...
import { adminBrandService } from "@/api";
...
export default function AdminBrandsPage() {
  ...
  const loadBrands = async () => {
    try {
      const data = await adminBrandService.getAll();
        ...
        <div className="container mx-auto max-w-4xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3">
            <div>
              ...
              <h1 className="text-lg font-bold uppercase tracking-tight text-primary">Brands</h1>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
              ...
              <div className="flex gap-2 md:justify-end col-span-1 md:col-span-3">
                ...
                >
                  ...
                  <span>{editingId ? "Update brand" : "Add brand"}</span>
                </button>
              </div>
            </form>
          </div>
              ...
              <table className="w-full text-left">
                <thead className="bg-secondary/30 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-[9px] uppercase tracking-wider font-bold text-gray-500">Slug</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {brands.map((b) => (
                    <tr key={b._id} className="hover:bg-secondary/20 transition-colors">
                      ...
                      <td className="px-3 py-2.5 text-[11px] text-gray-600 font-mono">{b.slug}</td>
                      <td className="px-3 py-2.5 text-[11px] text-gray-600">
                        {b.description || <span className="text-gray-400">—</span>}
                      </td>
